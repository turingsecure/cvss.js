...
/**
 * takes the vectorstring and cuts every not defined part out of it
 * 
 * @param {String} vectorString
 * @returns {String} vectorString with out unnecessary data
 */
function getCleanVectorString(vector){
  const stringParts = vector.split("/");
  var cleanString = "";
  var x = 0;
  while (stringParts[x] !== "MA:X" ){
    console.log(stringParts[x]);
    if (stringParts[x].search("X") === -1){
      if (stringParts[x].search("CVSS") === -1) stringParts[x] = "/" + stringParts[x];
      cleanString = cleanString.concat(stringParts[x]);
    }
    x++;
  }
  return cleanString;
