...
function CVSS(vector) {
  ...
  function getEnvironmentalScore() {
    ...
    if (!scopeChanged) {
      return util.roundUp(
        util.roundUp(Math.min(modifiedISC + modifiedExploitability, 10), 1) *
          eValue *
          rlValue *
          rcValue,
        1
  ...
  if (isVersionValid === "3.1") {
    throw new Error("The vector version 3.1 is not supported yet");
  }
  else if (isVersionValid === "Error") {
